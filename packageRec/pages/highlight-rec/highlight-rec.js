...
Page({
  ...
  /**
   * 检查监看健康度，当连续监看达到 20 分钟且在空闲状态下无感自动热重启 (Flush & Restart)
   */
  _checkPipelineHealthAndAutoRestart: function () {
    ...
    // 连续监看达到 20 分钟（20 * 60 * 1000 ms）阈值
    var autoRestartThresholdMs = 20 * 60 * 1000;
    ...
    this._dlog('REC', 'Auto soft-restart pipeline for health maintenance (20min idle threshold reached)');
